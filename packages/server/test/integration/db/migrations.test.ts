...
describe.skipIf(!DATABASE_URL)("drizzle migrations against a real Postgres", () => {
  ...
  it("creates exactly the eleven expected tables in the public schema", async () => {
    ...
    // Exactly these eleven -- KAN-1223's original four (workspaces,
    // workspace_members, specs, layouts), KAN-1226's Better Auth tables
    // (user, session, account, verification, invitation, sso_provider), and
    // KAN-1229's byok_keys (ADR-0016) -- no runs table sneaking in yet
    // (KAN-1231's), and no Drizzle-internal migrations-tracking table counted
    // (it lives in its own "drizzle" schema, not "public").

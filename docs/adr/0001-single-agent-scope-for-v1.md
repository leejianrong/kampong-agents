...
- Multi-agent org-chart orchestration is now an explicit (if not yet detailed) roadmap item, SLICES.md V7 — not a "someday maybe." It stays sequenced after the single-agent spec and execution model (V1–V4) are proven, since the org-chart canvas and delegation semantics are a real, separate design effort of their own.
